var ui={modalForm:{modalClass:undefined,formClass:undefined,error:$("#form_outer-equipment-and-location__error")},modalContainer:$(".modal-container"),showModalButton:$(".new-table-row")};function _showError(message){ui.modalForm.error.text(message);ui.modalForm.error.removeClass("d-none")}function _hideError(){ui.modalForm.error.addClass("d-none")}function getAgGridCurrentIdOuter(){if(ibpAgGrid.isReady===true){return ibpAgGrid.getCurrentIdOuter()}}function getInputsArr(){let inputValues=ui.modalForm.formClass.serializeArray();for(let i=inputValues.length-1;i>=0;i--){if(inputValues[i].value===""){inputValues.splice(i,1)}}return inputValues}function setFormSubmitHandler(form,postUrl,getUrl){console.log(getUrl);console.log(form);form.submit(event=>{event.preventDefault();let inputValues=getInputsArr();let currentIdOuter=getAgGridCurrentIdOuter();if(currentIdOuter!==undefined){inputValues.push({name:"id_outer",value:currentIdOuter})}$.ajax({url:postUrl,method:"POST",data:inputValues,contentType:"application/x-www-form-urlencoded",dataType:"json",success:()=>{_hideError();form.modal("hide");form.trigger("reset");if(ibpAgGrid.isReady===true){if(currentIdOuter==undefined){ibpAgGrid.setGridData(getData(getUrl))}else{ibpAgGrid.setGridData(getData(getUrl+"/"+currentIdOuter))}}actionMenu.hideOneRowAction()},error:function(response){console.log(response);_showError("Ошибка, попробуйте еще раз")}})})}function createModalEquipLocationList(data){let selectHtml="";$.each(data,function(key,val){selectHtml+=`<option>`+val.place_first_lev+`</option>`});$(".modal__place_first_lev-select").append(selectHtml)}function setModalInnerFormHtml(){var modalInnerEquip=`
...
`;ui.showModalButton.attr("data-bs-target","#modal-new-inner-equip");ui.modalContainer.html(modalInnerEquip);ui.modalForm.modalClass=$("#modal-new-inner-equip");ui.modalForm.formClass=$("#form_inner-equipment");setFormSubmitHandler(ui.modalForm.modalClass,config.api.postInnerEquipByOuterId,config.api.getInnerByOuterId)}function setModalOuterFormHtml(){var modalOuterEquipBuild=`
...
`;ui.showModalButton.attr("data-bs-target","#modal-new-outer-equip");ui.modalContainer.html(modalOuterEquipBuild);ui.modalForm.modalClass=$("#modal-new-outer-equip");ui.modalForm.formClass=$("#form_outer-equipment-and-location");setFormSubmitHandler(ui.modalForm.modalClass,config.api.postOuterEquipAndLocation,config.api.getDataBuildingAndOuter);var data=getData(config.api.getDataListOfObjects);createModalEquipLocationList(data)}